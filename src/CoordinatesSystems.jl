...
get_components(::AbstractCSVector{S},missing) where {S} = [s() for s in fieldtypes(S)]
...
get_components(p::AbstractPTensor{S}, missing) where {S} = [(s(),s_) for (s,sn) in zip(fieldtypes(S),fieldnames(S)) for s_ in get_components(getproperty(p,sn))]
